...
void Qentem::Document::_makeListNumber(Document &document, const String &content, const Match &item) noexcept {
    double  tn;
    UNumber end;
...
void Qentem::Document::_makeDocument(Document &document, const String &content, Array<Match> &items) noexcept {
    ...
    if (!document.Ordered) {
        ...
        if (items.Size > 0) {
            if (items.Storage[0].Expr->Keyword == L'"') { // Strings
            ...
            } else if (items.Storage[0].Expr->Keyword == L'}') { // Unordered arrays
            ...
            } else if (items.Storage[0].Expr->Keyword == L']') { // Ordered arrays
                Match *data;
                document.Documents.SetCapacity(items.Size);
